...
-- Allow new roles; keep 'user' for backward compatibility (treat as general)
...
-- Optional: migrate existing 'user' to 'general'
update public.profiles set role = 'general' where role = 'user';

-- Profiles: enable RLS so we can control who sees the list (superuser needs to list users)
...
-- Calculations: superuser can read all (to view any user's notes)
...
-- Stations: superuser can insert/update/delete
...
-- Vehicles: superuser can insert/update/delete
